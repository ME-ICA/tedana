...
def tedana_workflow(
    ...
    LGR.debug(
        "Retaining {}/{} samples for denoising".format(mask_denoise.sum(), n_samp)
    )
    ...
    LGR.debug(
        "Retaining {}/{} samples for classification".format(mask_clf.sum(), n_samp)
    )
    ...
    if t2smap is None:
        ...
        cap_t2s = stats.scoreatpercentile(
            t2s_full.flatten(), 99.5, interpolation_method="lower"
        )
        LGR.debug(
        ...
        if verbose:
            io_generator.save_file(
                utils.millisec2sec(t2s_limited), "limited t2star img"
            )
            io_generator.save_file(s0_limited, "limited s0 img")

    # optimally combine data
    data_oc = combine.make_optcom(
        catd, tes, masksum_denoise, t2s=t2s_full, combmode=combmode
    )
    ...
    if "mir" in gscontrol:
        gsc.minimum_image_regression(
            data_oc, mmix, mask_denoise, comptable, io_generator
        )
