...
def generate_metrics(
    ...
    if ("kappa rank" in required_metrics) or ("rho rank" in required_metrics):
        LGR.info("Ranking kappa and rho")
        comptable["rho_rank"] = comptable["rho"].rank(ascending=False).values
        comptable["kappa_rank"] = comptable["kappa"].rank(ascending=False).values
    ...
    if "variance explained rank" in required_metrics:
        LGR.info("Ranking variance explained")
        comptable["variance explained rank"] = (
            comptable["variance explained"].rank(ascending=False).values
        )
