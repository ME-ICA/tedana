...
def _spectrum_data_src(comptable, tr, n_comps):
    """
    Parameters
    ----------
    out_dir :
        The output directory of tedana
    tr : float
        Repetition time of the acquired ME data set
    n_comps : int
        Number of components generated by the decomposition
    ...
    spectrum, _ = utils.get_spectrum(data_src.data['C000'], tr)
    Nf = spectrum.shape[0]
...
comptable = opj(out_dir, 'meica_mix.1D')
df = io.load_comp_ts(comptable)
n_comps, n_vols = df.shape

# Load the component table
comptable_ds = create_data_struct(out_dir)

# generate the component spectrum
data_src, Nf = _spectrum_data_src(out_dir, tr, n_comps)
