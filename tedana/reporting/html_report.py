...
def _update_template_bokeh(
    ...
    # A tab is only rendered if it has content. Info, ICA and Carpet are always populated,
    # but the decay figures are not regenerated by every workflow and the decision tree
    # files are missing from older output directories.
    decay_exists = (
        adaptive_mask_exists or t2star_exists or s0_exists or variance_exists or rmse_exists
    )
...
def _flatten_tree_value(value):
    """Join list-valued decision tree fields into a single string."""
    if isinstance(value, list):
        return ", ".join(str(sub_value) for sub_value in value)
    return str(value)


def _generate_tree_tables(io_generator):
    """Generate HTML tables describing the decision tree and the component statuses.
    ...
    Returns
    -------
    tree_table : :obj:`str` or None
        HTML table of the decision tree nodes, or None if the decision tree json is absent.
    status_table : :obj:`str` or None
        HTML table of the component classification at each node, or None if the tsv is absent.
    """
    ...
    if os.path.isfile(tree_path):
        # n_true/n_false are only reported by nodes that make a decision, and node_label
        # is a list for some nodes, so neither can be accessed directly.
        rows = [
            {
                "Node": node["outputs"].get("decision_node_idx", ""),
                "Function": node.get("functionname", ""),
                "Decision": _flatten_tree_value(node["outputs"].get("node_label", "")),
                "Components true": node["outputs"].get("n_true", ""),
                "Components false": node["outputs"].get("n_false", ""),
                "Metrics used": _flatten_tree_value(node["outputs"].get("used_metrics", [])),
            }
            for node in load_json(tree_path).get("nodes", [])
        ]
        if rows:
            tree_table = pd.DataFrame(rows).to_html(index=False, border=0, classes=table_classes)
    else:
        LGR.warning("No decision tree json found, so not displaying it in the report")

    status_table = None
    status_path = io_generator.get_name("ICA status table tsv")
    if os.path.isfile(status_path):
        status_table = pd.read_csv(status_path, sep="\t").to_html(
            index=False, border=0, classes=table_classes
        )
    else:
