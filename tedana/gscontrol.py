...
def gscontrol_raw(catd, optcom, n_echos, generator, dtrank=4):
    """
    ...
    sphis -= sphis.mean()
    generator.save_file(utils.unmask(sphis, Gmask), "gs img")
    ...
    generator.save_file(glsig_df, "global signal time series tsv")
    ...
    generator.save_file(optcom, "has gs combined img")
    dm_optcom = utils.unmask(tsoc_nogs, Gmask)
    generator.save_file(dm_optcom, "removed gs combined img")
...
def minimum_image_regression(optcom_ts, mmix, mask, comptable, generator):
    """
    ...
    t1_map -= t1_map.mean()
    generator.save_file(utils.unmask(t1_map, mask), "t1 like img")
    ...
    generator.save_file(utils.unmask(hik_ts, mask), "ICA accepted mir denoised img")

    # Make denoised version of T1-corrected time series
    medn_ts = optcom_mean + ((mehk_noT1gs + resid) * optcom_std)
    generator.save_file(utils.unmask(medn_ts, mask), "mir denoised img")
    ...
    generator.save_file(mixing_df, "ICA MIR mixing tsv")
