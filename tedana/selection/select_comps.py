...
def selcomps(seldict, comptable, mmix, manacc, n_echos):
    ...
    for i_comp in acc:
        comptable.loc[i_comp, "dice_FR2"] = utils.dice(
            Br_R2_clmaps[:, i_comp], F_R2_clmaps[:, i_comp]
        )
        comptable.loc[i_comp, "dice_FS0"] = utils.dice(
            Br_S0_clmaps[:, i_comp], F_S0_clmaps[:, i_comp]
        )
    ...
    for i_comp in all_comps:
        comp_noise_sel = (np.abs(Z_maps[:, i_comp]) > 1.95) & (Z_clmaps[:, i_comp] == 0)
        comptable.loc[i_comp, "countnoise"] = np.array(
            comp_noise_sel, dtype=np.int
        ).sum()
        noise_FR2_Z = np.log10(np.unique(F_R2_maps[comp_noise_sel, i_comp]))
        signal_FR2_Z = np.log10(np.unique(F_R2_maps[Z_clmaps[:, i_comp] == 1, i_comp]))
        (
            comptable.loc[i_comp, "signal-noise_t"],
            comptable.loc[i_comp, "signal-noise_p"],
        ) = stats.ttest_ind(signal_FR2_Z, noise_FR2_Z, equal_var=False)
    ...
    temp_rej1 = all_comps[
        (comptable["dice_FS0"] > comptable["dice_FR2"])
    ...
    temp_rej2 = acc[
        (comptable.loc[acc, "signal-noise_t"] < 0)
        & (
    ...
    for nn in range(3):
        ncls = (
            comptable.loc[ncls]
            .loc[comptable.loc[ncls, "variance explained"].diff() < varex_upper_p]
            .index.values
        )
    ...
    kappa_elbow = np.min(
        (
            getelbow(kappas_lim, return_val=True),
    ...
    good_guess = ncls[
        (comptable.loc[ncls, "kappa"] >= kappa_elbow)
    ...
    kappa_rate = (
        np.max(comptable.loc[good_guess, "kappa"])
    ...
    midk = acc[
        (comptable.loc[acc, "d_table_score"] > max_good_d_score)
    ...
    if len(acc) > len(good_guess):
        ...
        d_table_rank = np.vstack(
            [
                ...
                len(acc)
                - stats.rankdata(
                    comptable.loc[acc, "signal-noise_t"], method="ordinal"
                ),
        ...
        num_acc_guess = int(
            np.mean(
                [
                    np.sum(
                        (comptable.loc[acc, "kappa"] > kappa_elbow)
        ...
        candartA = np.intersect1d(
            candartA,
            candartA[
                comptable.loc[candartA, "variance explained"]
        ...
        candartB = (
            ...
            .loc[
                ...
                > num_acc_guess * n_decision_metrics * HIGH_PERC / 100.0
            ]
            .index.values
        )
        candartB = np.intersect1d(
            candartB,
            candartB[
                comptable.loc[candartB, "variance explained"]
        ...
        candart = (
            comptable.loc[acc]
            .loc[
                comptable.loc[acc, "d_table_score"] > num_acc_guess * n_decision_metrics
            ]
            .index.values
        )
