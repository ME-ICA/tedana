...
@due.dcite(
    ...
    description="Introduces method for choosing PCA dimensionality " "automatically",
)
def run_mlepca(data):
    """
    ...
    u = np.dot(np.dot(data, v.T), np.diag(1.0 / s))
    return u, s, v
...
def kundu_tedpca(ct_df, n_echos, kdaw, rdaw, stabilize=False):
    ...
    ct_df : :obj:`pandas.DataFrame`
        ...
        or 'ignored'.
    """
    eigenvalue_elbow = getelbow(ct_df["normalized variance explained"], return_val=True)
    ...
    if int(kdaw) == -1:
        lim_idx = utils.andb([ct_df["kappa"] < fmid, ct_df["kappa"] > fmin]) == 2
        ...
        stabilize = True
        LGR.info("kdaw set to -1. Switching TEDPCA method to " "kundu-stabilize")
    elif int(rdaw) == -1:
        lim_idx = utils.andb([ct_df["rho"] < fmid, ct_df["rho"] > fmin]) == 2
        rho_lim = ct_df.loc[lim_idx, "rho"].values
        rho_thr = rho_lim[getelbow(rho_lim)]
...
def tedpca(
    ...
    if op.exists("pcastate.pkl"):
        ...
        if pcastate["method"] != method:
            LGR.warning(
                "Method from PCA state file ({0}) does not match "
                "requested method ({1}).".format(pcastate["method"], method)
            )
            state_found = False
        else:
            state_found = True
    ...
    if not state_found:
        if method == "mle":
            ...
            voxel_comp_weights = np.dot(np.dot(dz, comp_ts.T), np.diag(1.0 / varex))
    ...
    if method == "kundu":
        ...
        LGR.info(
            "Selected {0} components with MLE dimensionality "
    ...
    voxel_kept_comp_weighted = voxel_comp_weights[:, sel_idx] * varex[None, sel_idx]
...
def tedica(n_components, dd, fixed_seed, maxit=500, maxrestart=10):
    ...
    for i_attempt in range(maxrestart):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if len(w):
                LGR.warning(
                    "ICA attempt {0} failed to converge after {1} "
