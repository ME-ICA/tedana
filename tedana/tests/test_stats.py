...
def test_compute_least_squares():
    ...
    betas = compute_least_squares(data, X, mask=None, add_const=False)
    betas = np.squeeze(betas)
    assert np.allclose(betas, np.array([5., 5.]))

    betas = compute_least_squares(data, X, mask=None, add_const=True)
    betas = np.squeeze(betas)
    assert np.allclose(betas, np.array([5., 5.]))

    betas = compute_least_squares(data, X, mask=mask, add_const=False)
    betas = np.squeeze(betas)
    assert np.allclose(betas, np.array([5, 0]))

    betas = compute_least_squares(data, X, mask=mask, add_const=True)
    betas = np.squeeze(betas)
    assert np.allclose(betas, np.array([5, 0]))


def test_break_compute_least_squares():
    """
    Ensure that compute_least_squares fails when input data do not have the right
    shapes.
    """
    ...
    data = np.empty((n_samples))
    with pytest.raises(ValueError):
        compute_least_squares(data, X, mask, add_const=False)
...
def test_smoke_compute_least_squares():
    """
    Ensure that compute_least_squares returns outputs with different inputs and optional paramters
    """
    ...
    assert compute_least_squares(data_2d, x) is not None
    # assert compute_least_squares(data_3d, x) is not None TODO: submit an issue for the bug
    assert compute_least_squares(data_2d, x, mask=mask) is not None
    assert compute_least_squares(data_2d, x, add_const=True) is not None
