...
def test_basic_tedana():
    ...
    workflows.tedana_workflow(
        [op.expanduser("~/data/zcat_ffd.nii.gz")], [14.5, 38.5, 62.5]
    )
...
def compare_nifti(fn, test_dir, res_dir):
    ...
    passed = np.allclose(nib.load(res_fp).get_data(), nib.load(test_fp).get_data())
    return passed
...
def test_outputs():
    ...
    for fn in nifti_test_list:
        passed = compare_nifti(
            fn,
            Path(op.expanduser("~/data/TED/")),
