...
def test_break_computefeats2():
    ...
    assert str(e_info.value) == (
        "Parameter data should be 2d, not {0}d".format(data.ndim)
    )
    ...
    with pytest.raises(ValueError) as e_info:
        fit.computefeats2(data, mmix, mask, normalize=True)
    assert str(e_info.value) == (
        "Parameter mmix should be 2d, not {0}d".format(mmix.ndim)
    )
    ...
    with pytest.raises(ValueError) as e_info:
        fit.computefeats2(data, mmix, mask, normalize=True)
    assert str(e_info.value) == (
        "Parameter mask should be 1d, not {0}d".format(mask.ndim)
    )
