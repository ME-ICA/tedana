...
On this page, we describe the files that may be generated by ``tedana`` workflows, depending on the settings used.
...
.. warning::
  ...
  If you run ``tedana`` multiple times without varying the prefix or output directory, you will overwrite files from previous runs.
